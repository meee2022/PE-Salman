...
export default function ExamFollowupPage() {
  const { token, user } = useAuth();
  const YEAR = useActiveYear();
  const router = useRouter();
  const isAdmin = ["admin", "superadmin"].includes(user?.role ?? "");
  const forms = useQuery(api.examFollowupForms.list, token ? { academicYear: YEAR, token } : "skip");
  const supOptions = useQuery(api.supervisors.list, token && isAdmin ? { token } : "skip");
  const create = useMutation(api.examFollowupForms.create);
  const removeForm = useMutation(api.examFollowupForms.remove);

  const [showNew, setShowNew] = useState(false);
  const [saving, setSaving] = useState(false);
  const [hdr, setHdr] = useState({ schoolName: "", schoolId: undefined as string | undefined, supervisorId: undefined as string | undefined, supervisorName: "", teacherName: "", stage: "الابتدائية", term: "منتصف الفصل الدراسي الثاني", day: "الأحد", date: new Date().toISOString().slice(0, 10) });
  ...
  async function createForm() {
    ...
    try {
      const id = await create({
        ...
        teacherName: hdr.teacherName, stage: hdr.stage, term: hdr.term, day: hdr.day, date: hdr.date,
        academicYear: YEAR, domains: buildFollowupDomains(), token: token ?? undefined,
      });
      router.push(`/dashboard/exam-followup/${id}`);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="استمارة متابعة تنفيذ الاختبار" subtitle={isAdmin ? `${forms.length} استمارة` : "استمارات متابعة تنفيذ الاختبار"} icon={<ClipboardCheck size={26} />}
      ...
      {forms.length === 0 ? (
        ...
        <SupervisorFormList forms={rows} isAdmin={isAdmin} basePath="/dashboard/exam-followup" onDelete={(id) => { if (confirm("حذف الاستمارة؟")) removeForm({ id: id as Id<"examFollowupForms">, token: token ?? undefined }); }} searchPlaceholder="ابحث باسم المعلم أو المدرسة..." />
      )}

      {showNew && (
        <Modal title="استمارة متابعة تنفيذ اختبار جديدة" onClose={() => setShowNew(false)}>
