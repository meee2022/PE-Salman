...
import { TERMS } from "@/components/visitFormsTemplates";
import { GraduationCap, Plus } from "lucide-react";
import { useActiveYear } from "@/hooks/useActiveYear";

const WEEKDAYS = ["الأحد", "الاثنين", "الثلاثاء", "الأربعاء", "الخميس"];

export default function ProfDevPage() {
  const { token, user } = useAuth();
  const YEAR = useActiveYear();
  const router = useRouter();
  const isAdmin = ["admin", "superadmin"].includes(user?.role ?? "");
  const forms = useQuery(api.profDevForms.list, token ? { academicYear: YEAR, token } : "skip");
  const supOptions = useQuery(api.supervisors.list, token && isAdmin ? { token } : "skip");
  const create = useMutation(api.profDevForms.create);
  const removeForm = useMutation(api.profDevForms.remove);

  const [showNew, setShowNew] = useState(false);
  const [saving, setSaving] = useState(false);
  const [hdr, setHdr] = useState({ schoolName: "", schoolId: undefined as string | undefined, supervisorId: undefined as string | undefined, supervisorName: "", topic: "", term: "الفصل الدراسي الثاني", day: "الأحد", date: new Date().toISOString().slice(0, 10) });
  ...
  async function createForm() {
    ...
    try {
      const id = await create({
        ...
        topic: hdr.topic, term: hdr.term, day: hdr.day, date: hdr.date, academicYear: YEAR,
        supervisionTypes: [], supervisionMethods: [], recommendations: [], attendance: [], token: token ?? undefined,
      });
      router.push(`/dashboard/prof-dev/${id}`);
    } finally { setSaving(false); }
  }

  const rows = forms.map((f) => ({ _id: f._id, schoolName: f.schoolName, supervisorId: f.supervisorId, supervisorName: f.supervisorName, status: f.status, date: f.date, primary: f.topic || "تطوير مهني" }));

  return (
    <div className="space-y-6">
      <PageHeader title="استمارة التطوير المهني" subtitle={isAdmin ? `${forms.length} استمارة` : "استمارات التطوير المهني"} icon={<GraduationCap size={26} />}
      ...
      {forms.length === 0 ? (
        <div className="card-luxurious p-16 text-center text-stone-500 text-sm flex flex-col items-center justify-center gap-3">
          <GraduationCap className="w-12 h-12 text-gold/40 animate-pulse" />
        ...
        <SupervisorFormList forms={rows} isAdmin={isAdmin} basePath="/dashboard/prof-dev" onDelete={(id) => { if (confirm("حذف الاستمارة؟")) removeForm({ id: id as Id<"profDevForms">, token: token ?? undefined }); }} searchPlaceholder="ابحث بالموضوع أو المدرسة..." />
      )}

      {showNew && (
        <Modal title="استمارة تطوير مهني جديدة" onClose={() => setShowNew(false)}>
          <div className="p-6 space-y-4">
            ...
            <TxtField label="الموضوع" value={hdr.topic} onChange={(v) => setHdr({ ...hdr, topic: v })} placeholder="موضوع جلسة التطوير المهني..." />
