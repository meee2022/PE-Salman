...
import { buildDeputyDomains, TERMS, DEPUTY_FOLLOWUP_TYPES } from "@/components/visitFormsTemplates";
import { UserCog, Plus } from "lucide-react";
import { useActiveYear } from "@/hooks/useActiveYear";

const WEEKDAYS = ["الأحد", "الاثنين", "الثلاثاء", "الأربعاء", "الخميس"];

export default function DeputyPrincipalPage() {
  const { token, user } = useAuth();
  const YEAR = useActiveYear();
  const router = useRouter();
  const isAdmin = ["admin", "superadmin"].includes(user?.role ?? "");
  const forms = useQuery(api.deputyPrincipalForms.list, token ? { academicYear: YEAR, token } : "skip");
  const supOptions = useQuery(api.supervisors.list, token && isAdmin ? { token } : "skip");
  const create = useMutation(api.deputyPrincipalForms.create);
  const removeForm = useMutation(api.deputyPrincipalForms.remove);
  ...
  async function createForm() {
    if (!hdr.schoolName.trim() || !hdr.principalName.trim() || !hdr.deputyName.trim() || (isAdmin && !hdr.supervisorId)) return;
    setSaving(true);
    try {
      const id = await create({
        ...
        principalName: hdr.principalName, deputyName: hdr.deputyName,
        term: hdr.term, day: hdr.day, date: hdr.date,
        academicYear: YEAR, followUpTypes: ["ميدانية"],
        domains: buildDeputyDomains(), token: token ?? undefined,
      });
      router.push(`/dashboard/deputy-principal/${id}`);
    } finally { setSaving(false); }
  }

  const rows = forms.map((f) => ({ _id: f._id, schoolName: f.schoolName, supervisorId: f.supervisorId, supervisorName: f.supervisorName, status: f.status, date: f.date, primary: f.deputyName }));

  return (
    <div className="space-y-6">
      <PageHeader
        title="استمارة متابعة أداء نائب المدير للشؤون الأكاديمية"
        subtitle={isAdmin ? `${forms.length} استمارة` : "استمارات متابعة نائب المدير"}
      ...
      {forms.length === 0 ? (
        <div className="card-luxurious p-16 text-center text-stone-500 text-sm flex flex-col items-center justify-center gap-3">
          <UserCog className="w-12 h-12 text-gold/40 animate-pulse" />
        ...
        <SupervisorFormList
          ...
          basePath="/dashboard/deputy-principal"
          onDelete={(id) => { if (confirm("حذف الاستمارة؟")) removeForm({ id: id as Id<"deputyPrincipalForms">, token: token ?? undefined }); }}
          searchPlaceholder="ابحث باسم نائب المدير أو المدرسة..."
        />
      )}

      {showNew && (
        <Modal title="استمارة متابعة نائب المدير جديدة" onClose={() => setShowNew(false)}>
          <div className="p-6 space-y-4">
            ...
            <TxtField label="نائب المدير للشؤون الأكاديمية" required value={hdr.deputyName} onChange={(v) => setHdr({ ...hdr, deputyName: v })} placeholder="اكتب اسم نائب المدير" />
          ...
          <ModalFooter
            ...
            disabled={!hdr.schoolName.trim() || !hdr.principalName.trim() || !hdr.deputyName.trim() || (isAdmin && !hdr.supervisorId)}
          />
        </Modal>
      )}
    </div>
