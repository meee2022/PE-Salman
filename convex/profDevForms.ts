  ...
  handler: async (ctx, args) => {
    ...
    if (canSeeAll(user)) {
      rows = args.academicYear
        ? await ctx.db.query("profDevForms").withIndex("by_year", (q) => q.eq("academicYear", args.academicYear!)).collect()
        : await ctx.db.query("profDevForms").collect();
    } else if (user.role === "supervisor" && user.supervisorId) {
      rows = await ctx.db.query("profDevForms").withIndex("by_supervisor", (q) => q.eq("supervisorId", user.supervisorId!)).collect();
    } else return [];
...
export const get = query({
  args: { id: v.id("profDevForms"), token: v.optional(v.string()) },
  handler: async (ctx, args) => {
    const user = await getAuthUser(ctx, args.token);
...
export const recommendationsBank = query({
  ...
  handler: async (ctx, args) => {
    const user = await getAuthUser(ctx, args.token);
    if (!user) return [];
    const rows = await ctx.db.query("profDevForms").collect();
    const set = new Set<string>();
    for (const f of rows) for (const r of f.recommendations) { const t = r.trim(); if (t) set.add(t); }
...
export const create = mutation({
  args: {
    ...
    supervisionTypes: v.array(v.string()),
    supervisionMethods: v.array(v.string()),
  ...
  handler: async (ctx, args) => {
    ...
    const id = await ctx.db.insert("profDevForms", {
      ...
      day: args.day, date: args.date, term: args.term, academicYear: args.academicYear,
      supervisionTypes: args.supervisionTypes, supervisionMethods: args.supervisionMethods,
      topic: args.topic, summary: args.summary, recommendations: args.recommendations,
      attachments: args.attachments, attendance: args.attendance,
      status: "draft", createdBy: user._id, createdAt: Date.now(),
    });
    await logAudit(ctx, user, "create", "profDevForm", id, `إنشاء استمارة تطوير مهني: ${args.schoolName}`);
    return id;
  },
});
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await logAudit(ctx, user, "update", "profDevForm", id, `تعديل استمارة تطوير مهني: ${args.schoolName}`);
  },
});
...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await logAudit(ctx, user, "delete", "profDevForm", args.id, `حذف استمارة تطوير مهني: ${form.schoolName}`);
  },
});
