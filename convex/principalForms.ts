  ...
  handler: async (ctx, args) => {
    ...
    if (canSeeAll(user)) {
      rows = args.academicYear
        ? await ctx.db.query("principalForms").withIndex("by_year", (q) => q.eq("academicYear", args.academicYear!)).collect()
        : await ctx.db.query("principalForms").collect();
    } else if (user.role === "supervisor" && user.supervisorId) {
      rows = await ctx.db.query("principalForms").withIndex("by_supervisor", (q) => q.eq("supervisorId", user.supervisorId!)).collect();
    } else return [];
...
export const get = query({
  args: { id: v.id("principalForms"), token: v.optional(v.string()) },
  handler: async (ctx, args) => {
    const user = await getAuthUser(ctx, args.token);
...
export const notesBank = query({
  ...
  handler: async (ctx, args) => {
    const user = await getAuthUser(ctx, args.token);
    if (!user) return [];
    const rows = await ctx.db.query("principalForms").collect();
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const id = await ctx.db.insert("principalForms", {
    ...
    await logAudit(ctx, user, "create", "principalForm", id, `إنشاء استمارة مدير المدرسة: ${args.principalName} (${args.schoolName})`);
    return id;
  },
});
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await logAudit(ctx, user, "update", "principalForm", id, `تعديل استمارة مدير المدرسة: ${form.principalName}`);
  },
});
...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await logAudit(ctx, user, "delete", "principalForm", args.id, `حذف استمارة مدير المدرسة: ${form.principalName}`);
  },
});
