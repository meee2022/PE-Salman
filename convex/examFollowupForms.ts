  ...
  handler: async (ctx, args) => {
    ...
    if (canSeeAll(user)) {
      rows = args.academicYear
        ? await ctx.db.query("examFollowupForms").withIndex("by_year", (q) => q.eq("academicYear", args.academicYear!)).collect()
        : await ctx.db.query("examFollowupForms").collect();
    } else if (user.role === "supervisor" && user.supervisorId) {
      rows = await ctx.db.query("examFollowupForms").withIndex("by_supervisor", (q) => q.eq("supervisorId", user.supervisorId!)).collect();
    } else return [];
...
export const get = query({
  args: { id: v.id("examFollowupForms"), token: v.optional(v.string()) },
  handler: async (ctx, args) => {
    const user = await getAuthUser(ctx, args.token);
...
// بنك الملاحظات/التوصيات المستخدمة سابقاً
export const notesBank = query({
  args: { token: v.optional(v.string()) },
  handler: async (ctx, args) => {
    const user = await getAuthUser(ctx, args.token);
    if (!user) return [];
    const rows = await ctx.db.query("examFollowupForms").collect();
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const id = await ctx.db.insert("examFollowupForms", {
    ...
    await logAudit(ctx, user, "create", "examFollowupForm", id, `إنشاء استمارة متابعة اختبار: ${args.teacherName} (${args.schoolName})`);
    return id;
  },
});
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await logAudit(ctx, user, "update", "examFollowupForm", id, `تعديل استمارة متابعة اختبار: ${args.teacherName}`);
  },
});
...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await logAudit(ctx, user, "delete", "examFollowupForm", args.id, `حذف استمارة متابعة اختبار: ${form.teacherName}`);
  },
});
